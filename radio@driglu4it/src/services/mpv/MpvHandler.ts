...
// fix titles that were accidentally decoded as Latin‑1 (e.g. “Ã¼”).
// harmless on normal strings.
function decodeMetadataString(str: string): string {
  try {
    const bytes = new Uint8Array(Array.from(str, (c) => c.charCodeAt(0)));
    // TextDecoder may not be a known symbol during TS compile; grab it
    // from the global object at runtime.
    const Decoder = (globalThis as any).TextDecoder;
    if (Decoder) {
      return new Decoder("utf-8", { fatal: false }).decode(bytes);
    }
    // fallback to old mechanism if decoder missing
...
function createMpvHandler() {
  ...
  control.connect("stream-added", (ctrl, id) => {
    ...
    if (addedStream?.name !== MPV_CVC_NAME) return;
  ...
  function activateMprisPropsListener(): void {
    mediaPropsListenerId = mediaProps.connectSignal(
      ...
      (proxy, nameOwner, [interfaceName, props]) => {
        ...
        // some streams send UTF-8 data which ends up being
        // interpreted as ISO-8859-1/ASCII by the MPRIS plugin.
        // In that case characters like "ü" become "Ã¼" etc.
        // We attempt to repair those by running the string
        // through a decoder; if the transformation fails we
  ...
  function getLength(): number {
    const lengthMicroSeconds =
      mediaServerPlayer.Metadata?.["mpris:length"]?.unpack() || 0;
...
>>>>>>> a5c7a16a3 (Refactor MpvHandler tests for improved readability and functionality)
...
=======
  ...
  function getVolume(props?: {
    ...
    return props?.dimension === "fraction"
      ? volumeFraction
  ...
  return {
    ...
    addPlaybackStatusChangeHandler: (
      changeHandler: ChangeHandler<AdvancedPlaybackStatus>,
    ) => {
      playbackStatusChangeHandler.push(changeHandler);
    },
    ...
    addLengthChangeHandler: (
      changeHandler: ChangeHandler<number | undefined>,
    ) => {
      lengthChangeHandler.push(changeHandler);
    },
