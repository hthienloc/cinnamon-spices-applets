...
const { resetInterfaces, simulateMpvStart, simulateTitleChanged } =
  initMprisMocks({ loadingTime: MOCKED_LOADING_TIME });
...
describe("Starting mpv with valid url is working", () => {
  async function simulateValidMpvStart(title?: string) {
    const args: any = {
      url: VALID_URL_1,
      volume: VALID_VOLUME_PERCENT_1,
    };
    if (title !== undefined) args.title = title;
    await simulateMpvStart(args);
  }
  ...
  it("should repair incorrectly encoded UTF-8 titles", async () => {
    const handler = initMpvListener({});
    await simulateValidMpvStart("MÃ¼ller - Ã¼ber");
...
describe("changing volume is working", () => {
  ...
  it("setting Volume udpates cvc Volume", async () => {
    ...
    const expectedCvcVolume =
      (control.get_vol_max_norm() * VALID_VOLUME_PERCENT_2) / 100;
